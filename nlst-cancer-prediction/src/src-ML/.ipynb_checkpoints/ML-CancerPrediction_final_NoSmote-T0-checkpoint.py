...
# get the start time
st = time.time()
file_name = 'cancerprediction_rads_T0_lasso_NoSmote'
#Load the data
main_dir= "/home/ulaval.ca/lesee/projects/Project-NLST/"

data_train = pd.read_excel(os.path.join(main_dir,'data/data_radiomics/radiomicsfeatures_kheops-NLST-Dmitrii-Cohort1_Laptop_v2-cleaned-T0-ML.xlsx'))
data_test = pd.read_excel(os.path.join(main_dir,'data/data_radiomics/radiomicsfeatures_kheops-NLST-Dmitrii-Cohort2_Laptop_v2-cleaned-T0-ML.xlsx'))
...
#        print(f"Best Parameters for {type(model).__name__} with {method}: {best_params}, AUC in CV: {best_score_cv}")
    ...
    df_scores_method.to_csv(os.path.join(main_dir, 'results/'+file_name+'/'+model_name+'/'+ model_name+'_scores_with_NoSmote.csv'), index=False, float_format='%.7f') 
