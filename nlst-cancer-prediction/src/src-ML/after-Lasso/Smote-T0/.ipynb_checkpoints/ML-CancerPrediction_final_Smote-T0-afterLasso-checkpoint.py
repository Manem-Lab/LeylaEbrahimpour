...
# get the start time
st = time.time()
file_name = 'cancerprediction_rads_T0_lasso_Smote_combat_manufacturer_notdropped'
...
df_training_data = pd.read_csv(os.path.join(main_dir,'data/data_lasso/T0/extra/Training-Lasso-scaled-combat-manufacturer_notdropped-T0.csv'))
df_test_data = pd.read_csv(os.path.join(main_dir,'data/data_lasso/T0/extra/Test-Lasso-scaled-combat-manufacturer_notdropped-T0.csv'))
...
#        print(f"Best Parameters for {type(model).__name__} with {method}: {best_params}, AUC in CV: {best_score_cv}")
    ...
    df_scores_method.to_csv(os.path.join(main_dir, 'results/results-rest/'+file_name+'/'+model_name+'/'+ model_name+'_scores_rads_T0_Lasso_Smote.csv'), index=False, float_format='%.7f') 
