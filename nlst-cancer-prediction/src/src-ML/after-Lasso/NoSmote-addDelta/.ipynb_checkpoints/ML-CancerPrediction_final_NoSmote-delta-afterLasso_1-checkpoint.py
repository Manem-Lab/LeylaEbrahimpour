...
file_name = 'cancerprediction_rads_T0+DeltaT2_lasso_NoSmote_combat_KT_part1'
...
df_training_data = pd.read_csv(os.path.join(main_dir,'data/data_lasso/T0+deltaT2/Training-Lasso-scaled-combat-kerneltype_notdropped-BaseAddDelta2.csv'))

#df_training_data = pd.read_csv(os.path.join(main_dir,'data/Training-Lasso-scaled-BaseAddDelta.csv'))

df_test_data = pd.read_csv(os.path.join(main_dir,'data/data_lasso/T0+deltaT2/Test-Lasso-scaled-combat-kerneltype_notdropped-BaseAddDelta2.csv'))
#df_test_data = pd.read_csv(os.path.join(main_dir,'data/Test-Lasso-scaled-BaseAddDelta.csv'))
...
#        print(f"Best Parameters for {type(model).__name__} with {method}: {best_params}, AUC in CV: {best_score_cv}")
    ...
    df_scores_method.to_csv(os.path.join(main_dir, 'results/results-rest/'+file_name+'/'+model_name+'/'+ model_name+'_scores.csv'), index=False, float_format='%.7f') 
