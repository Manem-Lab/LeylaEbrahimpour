...
# get the start time
st = time.time()
file_name = 'cancerprediction_T0+DeltaT1_lasso_Smote-combat-KT-notdropped-GB'
...
df_training_data = pd.read_csv(os.path.join(main_dir,'data/data_lasso/T0+deltaT1/Training-Lasso-scaled-combat-kerneltype_notdropped-BaseAddDelta1.csv'))
df_test_data = pd.read_csv(os.path.join(main_dir,'data/data_lasso/T0+deltaT1/Test-Lasso-scaled-combat-kerneltype_notdropped-BaseAddDelta1.csv'))
...
#        print(f"Best Parameters for {type(model).__name__} with {method}: {best_params}, AUC in CV: {best_score_cv}")
    ...
    df_scores_method.to_csv(os.path.join(main_dir, 'results/results-rest/'+file_name+'/'+model_name+'/'+ model_name+'_scores_rads_Delta1_Lasso_Smote.csv'), index=False, float_format='%.7f') 
