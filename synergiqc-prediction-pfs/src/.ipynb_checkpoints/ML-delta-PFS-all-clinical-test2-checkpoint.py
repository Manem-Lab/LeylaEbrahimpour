...
file_name = 'delta-PFS_st1_2_rad_clinical_test2'
#Load the data
main_dir= "/home/ulaval.ca/lesee/projects/Project2-synergiqc/PFS/"
...
df_training_data = pd.read_csv(os.path.join(main_dir,'data/delta-train_data_pfs_st1_2_rad.csv'))
df_test_data = pd.read_csv(os.path.join(main_dir,'data/delta-test_data_pfs_st1_2_rad.csv'))

df_training_data_clinical = pd.read_csv(os.path.join(main_dir,'data/delta-train_data_pfs_st1_2_clinical.csv'))
df_test_data_clinical = pd.read_csv(os.path.join(main_dir,'data/delta-test_data_pfs_st1_2_clinical.csv'))
...
X_train_clinical = df_training_data_clinical.values  # Select all columns except the last one
X_test_clinical = df_test_data_clinical.values  # Select all columns except the last one
...
cv = RepeatedKFold(n_splits=5, random_state=42, n_repeats=3)  
...
#                })
                ...
                results['n_fold'] = i+1
#                c_indices_grids_fold_n_feautres = []
#                for param in grid_search.cv_results_['params']:
#                    model_grid_fold = model.set_params(**param)
                    # Train the model on the training data in the current fold for each grid
#                    model_grid_fold.fit(X_train_fold_new, y_train_fold)
#                    y_prediction_grid_fold = predict_with_model(X_val_fold_new, model_grid_fold)
                    # Calculate the c-index for the predictions on the test dataset in the current fold for each grid
#                    c_index_validation_grid_fold = concordance_index(y_val_fold, y_prediction_grid_fold)
#                    c_indices_grids_fold_n_feautres.append(c_index_validation_grid_fold)
#                results ['c_index'] = c_indices_grids_fold_n_feautres            
...
#        cv = RepeatedKFold(n_splits=5, random_state=42, n_repeats=3)  
        ...
        X_train_final = pd.concat([X_train_selected_final_df, X_train_clinical_df], axis=1)
        X_test_final = pd.concat([X_test_selected_final_df, X_test_clinical_df], axis=1)  
...
#        cv_scores = cross_val_score(model_selected, X_train_selected_final, y_train, cv=10, scoring=my_func) 
#        c_index_discovery = cv_scores.mean()
