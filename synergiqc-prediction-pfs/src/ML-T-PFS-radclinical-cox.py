...
file_name = 'T-PFS_st1_2_all_rad_clinical_corr_log_harmonized_cox_opposite'
...
df_training_data = pd.read_csv(os.path.join(main_dir,'data/T-train_data_pfs_st1_2_rad_corr_log_harmonized.csv'))
df_test_data = pd.read_csv(os.path.join(main_dir,'data/T-test_data_pfs_st1_2_rad_corr_log_harmonized.csv'))

#load the clinical data for the same training and test dataset that have been created by dataset_feature_type.py code
df_training_data_clinical = pd.read_csv(os.path.join(main_dir,'data/T-train_data_pfs_st1_2_clinical_corr_log_harmonized.csv'))
df_test_data_clinical = pd.read_csv(os.path.join(main_dir,'data/T-test_data_pfs_st1_2_clinical_corr_log_harmonized.csv'))
...
y_train_df = pd.DataFrame(y_train, columns=['PFS-train'])
...
#    method_names = []
    ...
    for index, n_features in enumerate(iteration_features):
        ...
        for i, (train_idx, val_idx) in enumerate(cv.split(X_train_selected)):
            ...
            y_train_fold_df = pd.DataFrame(y_train_fold, columns=['PFS-train'])
...
#                        method, X_train_fold, y_train_fold, X_val_fold, n_features)
            ...
            cox_model.fit(merged_df, duration_col='PFS-train', event_col='vital_status_train')
...
#    X_train_selected_final, X_test_selected_final, selector = cox_select_features(X_train_selected, y_train, X_test_selected, n_features_max, event_train_df)
    ...
    cox_model_train.fit(merged_df_train, duration_col='PFS-train', event_col='vital_status_train')
