...
for num_std , study_dicom in enumerate(studies_dicom): 
    ...
    #            studies_two_seg.append(patient_name)
            ...
            # Find all labels that start with "Non-Tumor"
            non_tumor_labels = [label for label in label_name if label.startswith("N")]            

            # Check if there is exactly one "Tumor" label
            if len(tumor_labels) == 1 and len(non_tumor_labels) >= 1:
                tumor_label = tumor_labels[0]
    ...
    #                tumor_segment_number = label_number[label_name.index(tumor_label)]     
                ...
                #   features = extractor.execute(img_CT_stk, img_seg_resampled, label=int(labels_resampled[-1]))
                    features = extractor.execute(img_CT_stk, img_seg, label=int(non_tumor_segment_number))
...
# write into excel      
writer = pd.ExcelWriter(os.path.join(main_dir,'results/radiomicsfeatures_kheops_SynergiQc_NT_test_normal.xlsx'), engine='xlsxwriter')
