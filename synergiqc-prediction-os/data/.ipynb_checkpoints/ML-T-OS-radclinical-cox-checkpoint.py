...
file_name = 'T-OS_st1_2_all_rad_clinical_corr_log_harmonized'
...
X_train_clinical_df = df_training_data_clinical.iloc[:, :-1].values  # Select all columns except the last one (training clinical data)
X_test_clinical_df = df_test_data_clinical.iloc[:, :-1].values  # Select all columns except the last one (test clinical data)

event_train_df = df_training_data_clinical.iloc[:, -1].values #Select only the last column
event_test_df = df_test_data_clinical.iloc[:, -1].values #Select only the last column
...
X_train_clinical_df = pd.DataFrame(X_train_clinical_df, columns=clinical_columns)
X_test_clinical_df = pd.DataFrame(X_test_clinical_df, columns=clinical_columns)
"""
...
#    method_names = []
    ...
    for index, n_features in enumerate(iteration_features):
        ...
        for i, (train_idx, val_idx) in enumerate(cv.split(X_train_selected)):
            ...
            event_train_fold, event_val_fold = event_train_df [train_idx],event_train_df[val_idx]
...
#                        method, X_train_fold, y_train_fold, X_val_fold, n_features)
            ...
            X_train_fold_new, X_val_fold_new, selector = cox_select_features(X_train_fold, y_train_fold, X_val_fold, n_features, event_train_fold)
